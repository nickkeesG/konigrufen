...
To be able to observe increasing knowledge in the simulation we use theframework of Public Announcement Logic (PAL), a framwork often used in multi-agent systems for modelling knowledge and announcements. For the definition of public announcement logic and what comes with it we refer to Chapter 4 in the book *Dynamic Epistemic Logic* by Hans van Ditmarsch. The only way players may 'communicate' about the state of the game once the round starts (after a player has already decided to make a play and called a king) is by playing a card. This is a useful way of looking at this as we can then treat the playing of a card as a public announcement. This allows us to update our kripke models representing knowledge using these public announcements as a basis to do so.
